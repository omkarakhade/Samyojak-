...
export default function ChoosePlan() {
  ...
  const handleSelect = async (productId: string, planName: string) => {
    ...
    try {
      const res = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({ productId, email: userEmail, name: userName, planName }),
      })
      const data = await res.json()
      if (data.url) {
        window.location.href = data.url
      } else {
        alert('Payment setup in progress. Contact hello@samyojak.app')
      }
    } catch (e) {
      console.error(e)
      alert('Something went wrong. Contact hello@samyojak.app')
    }
    setLoadingPlan('')
  }
  ...
  const regionLabel = {
    india: '🇮🇳 India pricing applied',
    western: '🌎 Western pricing applied',
    global: '🌍 Global pricing applied',
  }[region]
    ...
    <div className="min-h-screen py-12 px-4" style={{ background: '#FFFDF5' }}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold" style={{ background: regionColor.bg, border: `2px solid ${regionColor.border}`, color: regionColor.color }}>
            {regionLabel}
          </div>
        </div>
          ...
          {plans.map(plan => {
            ...
            return (
              ...
              >
                ...
                <div className="mb-3">
                  <span className="text-4xl font-black" style={{ fontFamily: 'Outfit', color: plan.popular ? 'white' : '#1E293B' }}>{plan.display_price}</span>
                ...
                {plan.lockedModules.length > 0 && (
                  <div className="mb-4 p-2 rounded-lg" style={{ background: plan.popular ? 'rgba(0,0,0,0.15)' : '#F8FAFC', border: `1px solid ${plan.popular ? 'rgba(255,255,255,0.2)' : '#E2E8F0'}` }}>
                    <p className="text-xs font-semibold mb-1" style={{ color: plan.popular ? 'rgba(255,255,255,0.6)' : '#94A3B8' }}>🔒 Locked in this plan:</p>
                    <p className="text-xs" style={{ color: plan.popular ? 'rgba(255,255,255,0.5)' : '#CBD5E1' }}>
                      {plan.lockedModules.map(m => m.charAt(0).toUpperCase() + m.slice(1)).join(', ')}
                    </p>
                  </div>
                )}
                ...
                >
                  {isLoading ? 'Loading...' : <><ArrowRight size={16} /> Select Plan</>}
                </button>
              </div>
            )
          })}
