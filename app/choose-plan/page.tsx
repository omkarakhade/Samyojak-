...
export default function ChoosePlan() {
  ...
  const handleSelect = (
    ...
    router.push(
      `/checkout?plan=${encodeURIComponent(planName)}&product=${encodeURIComponent(productId)}&price=${encodeURIComponent(price)}&period=${encodeURIComponent(period)}&bonus=${encodeURIComponent(bonus)}`
    )
  }
    ...
    <div className="min-h-screen py-12 px-4" style={{ background: '#FFFDF5' }}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-lg mb-4" style={{ color: '#64748B', fontFamily: 'Plus Jakarta Sans' }}>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-10">
          {[
            { label: '📅 Weekly', bonus: '+1 week free' },
            { label: '📆 Monthly', bonus: '+1 month free' },
            { label: '🗓️ Yearly', bonus: '+2-3 months free' },
          ].map(item => (
            <div
              key={item.label}
        ...
        >
          ...
          {region === 'india' && ' · Special India pricing applied'}
        </p>

        {/* Powered By */}
        <div className="flex items-center justify-center gap-2 mt-4 text-xs" style={{ color: '#CBD5E1' }}>
          <span>Powered by</span>
          <span className="font-black text-gray-700">▲ Vercel</span>
        </div>
      </div>
    </div>
  )
